...
```
Seurat tutorials/test data is ditributed through seurat objects.  These datasets can be downloaded using `SeuratData` package. To install `seuratData` and check for the datasets that are availble to download we need `devtools` pacakge.
...
# List all datasets that are available to download
AvailableData()

# Checking the first 10 with 5 columns
head(AvailableData(),10)[,1:5]
```

The 10th row has `ifnb`
...
nk.markers <- Seurat::FindConservedMarkers(immune.combined, ident.1 = 6, grouping.var = "stim", verbose = FALSE)
head(nk.markers)
```
