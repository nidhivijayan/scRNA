...
```{R}
head(fread("./WT1_filtered_feature_bc_matrix/WT1_features.tsv.gz"))
```
`fread` is a function from  `data.table` package that can read `.gz` compressed files and then head prints the first 6 rows.  So here we see that the first column contains mouse Ensemble geneID, second column have their common names and lastly third column indicates nature of the feature.

```{R}
head(fread("./WT1_filtered_feature_bc_matrix/WT1_barcodes.tsv.gz"),10)
```
...
wt1.data<-Read10xprefix(data.dir="./WT1_filtered_feature_bc_matrix/", prefix="WT1_")

#Read files withour prefix
#wt1.data<-Read10x(data.dir="./WT1_filtered_feature_bc_matrix/")
# To read in .h5 file formats
...
wt1.data[1:5,1:5]
```
So we have created a dense matrix (also refered as **countmatrix**)  of 32285 rows (# of genes) and 11481 columns (cells).
...
wt1Filtered2<-subset(wt1,
            subset=(nFeature_RNA >=200) & (nCount_RNA >=500) & (nCount_RNA < 20000) & (percent.mt < 20))# & (GenesPerUMI > 0.80))

```

**Gene Level Filtering**
Note: This is applicable if we have not done filtering while creating Seurat object. 
...
wt1Filtered <- CreateSeuratObject(filtered_counts, project="WT1")
...
wt3Filtered<-subset(wt1Filtered,
            subset=(nFeature_RNA >=200) & (nCount_RNA >=500) & (percent.mt < 20))

ggplot(wt3Filtered@meta.data,aes(x=nFeature_RNA,y=nCount_RNA,col=percent.mt))+
  geom_point()+
  scale_x_log10() + 
  scale_y_log10() +
  stat_smooth(method=lm,col="yellow")+
  geom_vline(xintercept=300)+geom_hline(yintercept=500)+geom_vline(xintercept=200,col="red")
```
...
```{R}
#save(wt1Filtered,counts, file="WT1_filtered.rdata")
#load("WT1_filtered.rdata")

saveRDS(wt1Filtered, file="WT1_filtered.rds")
